...
typedef struct
{
    ...
    /*! \brief Mutex lock for accessing user's channels */
...
typedef struct
{
    ...
    /*! \brief mutex lock for accessing users in the channel */
...
/*! \struct chirc_mutex_t
 * \brief Struct containing mutex locks for the context
 *
 * This struct contains all the locks used to maintain thread
 * saftey in a given context. It contains locks for the following
 * shared pointers:
 * */
typedef struct
{
...
typedef struct
{
    ...
    /*! \brief Mutex structure to protect shared data structures */
    chirc_mutex_t locks;
