...
//Send message to everyone in the channel (optionally including the user)
void send_msg_to_channel(chirc_ctx_t *ctx, chirc_connection_t *conn, chirc_message_t *msg, chirc_channel_t *target_channel, bool include_sender);

/**! \brief Constructs a "prefix reply" from a user
 * Constructs a message of the form:
 *  sprintf(prefix, "%s!%s@%s", user->nick, user->username, user->hostname);
 * As this is used often by the server to announce when a user has performed a command.
 * 
 * returns 1 for 
 * \param msg target message
 * \param user targer user
 * \param cmd string of the command to be relayed
*/
int chirc_message_contruct_user_prefix_reply(chirc_message_t *msg, chirc_user_t *user, sds cmd);
