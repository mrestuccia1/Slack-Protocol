...
void chirc_ctx_init(chirc_ctx_t *ctx)
{
    ...
    pthread_mutex_init(&ctx->locks.channel_lock, NULL);
    pthread_mutex_init(&ctx->locks.connection_lock, NULL);
    pthread_mutex_init(&ctx->locks.user_lock, NULL);
    pthread_mutex_init(&ctx->locks.server_lock, NULL);
}
...
void chirc_ctx_free(chirc_ctx_t *ctx)
{
    ...
    pthread_mutex_destroy(&ctx->locks.channel_lock);
    pthread_mutex_destroy(&ctx->locks.connection_lock);
    pthread_mutex_destroy(&ctx->locks.user_lock);
    pthread_mutex_destroy(&ctx->locks.server_lock);
