...
/* See message.h */
void send_msg_to_channel(chirc_ctx_t *ctx, chirc_connection_t *conn, chirc_message_t *sendMsg, chirc_channel_t *target_channel, bool include_sender) {
    chirc_user_t *user = conn->peer.user;
    chirc_channeluser_t *current_user;
    chirc_channeluser_t *tmp;

    HASH_ITER(hh_from_channel, target_channel->users, current_user, tmp) {
        if ((current_user->user->nick != user->nick) || include_sender) {
            if (current_user->user->conn) {
                chirc_connection_send_message(ctx, current_user->user->conn, sendMsg, false);
            }
        }
    }

    chirc_message_free(sendMsg);
}